...
class AppState extends ChangeNotifier {
  ...
  Future<String?> setLineup({
    ...
    if (m.status != MatchStatus.scheduled) {
      return 'Başlamış bir maçın kadrosu değiştirilemez.';
    }
    final team = findTeam(isHome ? m.homeTeamId : m.awayTeamId);
    if (team == null) return 'Takım bulunamadı.';
    final ids = playerIds.toSet();
    if (ids.length != playerIds.length) return 'Aynı oyuncu iki kez seçilemez.';
    if (playerIds.length > 11) return 'İlk 11 en fazla 11 oyuncu olabilir.';
    if (playerIds.any((id) => !team.players.any((p) => p.id == id))) {
      return 'Kadrodaki oyunculardan biri bu takıma ait değil.';
    }
    if (keeperId != null && !team.keepers.any((k) => k.id == keeperId)) {
      return 'Seçilen kaleci bu takıma ait değil.';
    }
  ...
  void _simulateShootoutIfRequired(MatchGame match) {
    ...
    final league = findLeague(match.leagueId);
    if (league == null || !league.allowPenalties) return;
