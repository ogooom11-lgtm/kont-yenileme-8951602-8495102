...
class InsightsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (final match in finished) {
      match.scorerMap.forEach((id, count) => scorers.update(id, (v) => v + count, ifAbsent: () => count));
    }
    ...
    String playerName(String id) {
      for (final team in state.teams) {
        for (final player in team.players) {
          if (player.id == id) return '${team.icon} ${player.fullName}';
        }
      }
        ...
        children: [
          ...
          Row(
            children: [
              ...
              _MetricCard('Tamamlanan', '${finished.length}', Icons.check_circle, Colors.green),
              const SizedBox(width: 8),
              _MetricCard('Gol', '$goals', Icons.sports_soccer, Colors.orange),
            ],
          ),
          const SizedBox(height: 20),
          const _Header('Gol krallığı', Icons.emoji_events_outlined),
          const SizedBox(height: 8),
          if (topScorers.isEmpty)
            _EmptyCard('Sonuçlara olay ekledikçe gol krallığı burada görünür.'),
          ...topScorers.take(5).toList().asMap().entries.map((entry) {
            final rank = entry.key;
            final scorer = entry.value;
            return Card(
              child: ListTile(
                leading: CircleAvatar(
                  backgroundColor: rank == 0 ? const Color(0xFFE8B923) : null,
                  child: Text('${rank + 1}'),
                ),
                title: Text(playerName(scorer.key), style: const TextStyle(fontWeight: FontWeight.w700)),
                trailing: Text('${scorer.value} gol', style: const TextStyle(fontWeight: FontWeight.w800)),
              ),
            );
          }),
          const SizedBox(height: 18),
...
class _InsightHero extends StatelessWidget {
  ...
  final int finished;
  const _InsightHero({required this.leagueCount, required this.matchCount, required this.finished});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        children: [
          const Icon(Icons.auto_awesome, color: Color(0xFFE8B923), size: 34),
          const SizedBox(width: 14),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('Sezonun nabzı', style: TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.w800)),
                const SizedBox(height: 4),
                Text('$leagueCount turnuva · $matchCount maç · $finished tamamlandı', style: const TextStyle(color: Colors.white70)),
              ],
            ),
          ),
        ],
